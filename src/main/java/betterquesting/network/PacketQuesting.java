...
public class PacketQuesting implements IMessage {
    ...
    public static class HandleServer implements IMessageHandler<PacketQuesting, IMessage> {
        ...
        public IMessage onMessage(PacketQuesting packet, MessageContext ctx) {
            ...
            final NBTTagCompound message = PacketAssembly.INSTANCE.assemblePacket(
                    sender == null ? null : QuestingAPI.getQuestingUUID(sender), packet.tags);
            ...
            final Consumer<Tuple2<NBTTagCompound, EntityPlayerMP>> method =
                    PacketTypeRegistry.INSTANCE.getServerHandler(new ResourceLocation(message.getString("ID")));

            if (method == null) {
                BetterQuesting.logger.log(
                        Level.WARN, "Recieved a packet server side with an invalid ID: " + message.getString("ID"));
                return null;
            } else if (sender != null) {
    ...
    public static class HandleClient implements IMessageHandler<PacketQuesting, IMessage> {
        ...
        public IMessage onMessage(PacketQuesting packet, MessageContext ctx) {
            ...
            final Consumer<NBTTagCompound> method =
                    PacketTypeRegistry.INSTANCE.getClientHandler(new ResourceLocation(message.getString("ID")));
