...
public final class PacketAssembly {
  ...
  public NBTTagCompound assemblePacket(UUID owner, NBTTagCompound tags) {
    ...
    if (end) {
      ...
      try {
        DataInputStream dis =
            new DataInputStream(new BufferedInputStream(new GZIPInputStream(new ByteArrayInputStream(tmp))));
      ...
      synchronized (buffer) {
        if (buffer.containsKey(owner)) {
          throw new IllegalStateException(
              "Attepted to start more than one BQ packet assembly for UUID " + owner);
        }
