...
public class NetQuestEdit {
    ...
    public static void sendEdit(
            NBTTagCompound
                    payload) // TODO: Make these use proper methods for each action rather than directly assembling the
                // payload
            {
        ...
        for (DBEntry<IQuest> entry : questList) {
            ...
            if (player != null)
                BetterQuesting.logger.info(
                        "{} ({}) completed quest {}", player.getDisplayName(), targetID, entry.getID());
        }
