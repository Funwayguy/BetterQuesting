...
public class NetImport {
    ...
    private static void onServer(Tuple2<NBTTagCompound, EntityPlayerMP> message) {
        ...
        BetterQuesting.logger.log(
                Level.INFO,
                "Importing " + impQuestDB.size() + " quest(s) and " + impQuestLineDB.size() + " quest line(s) from "
                        + sender.getGameProfile().getName());

        HashMap<Integer, Integer> remapped = getRemappedIDs(impQuestDB.getEntries());

        for (DBEntry<IQuest> entry : impQuestDB.getEntries()) {
            int[] oldIDs = Arrays.copyOf(
                    entry.getValue().getRequirements(), entry.getValue().getRequirements().length);
            ...
            for (DBEntry<IQuestLineEntry> qle : pendingQLE) {
                if (!remapped.containsKey(qle.getID())) {
                    BetterQuesting.logger.error(
                            "Failed to import quest into quest line. Unable to remap ID " + qle.getID());
                    continue;
                }
