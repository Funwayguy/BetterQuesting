...
public class NBTConverter {
  ...
  private static NBTBase JSONtoNBT_Element(JsonElement jObj, byte id, boolean format) {
    ...
    QuestingAPI.getLogger()
               .log(Level.WARN, "Unknown NBT representation for " + jObj + " (ID: " + tagID + ")");
    return new NBTTagString();
  }
  ...
  private static byte fallbackTagID(JsonElement jObj) {
    ...
    if (jObj.isJsonPrimitive()) {
      ...
      for (JsonElement entry : array) {
        if (entry.isJsonPrimitive() && tagID ==
                                       0) // Note: TagLists can only support Integers, Bytes and Compounds (Strings can be stored but require special handling)
        {
          try {
