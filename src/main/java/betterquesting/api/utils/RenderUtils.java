...
public class RenderUtils {
    ...
    public static void RenderItemStack(Minecraft mc, ItemStack stack, int x, int y, float z, String text, int color) {
        ...
        itemRender.zLevel =
                -50F; // Counters internal Z depth change so that GL translation makes sense // NOTE: Slightly different
        // depth in 1.7.10
    ...
    public static void RenderEntity(
            float posX, float posY, float posZ, int scale, float rotation, float pitch, Entity entity) {
        try {
            GL11.glEnable(GL11.GL_COLOR_MATERIAL);
            GL11.glPushMatrix();
            GL11.glEnable(GL11.GL_DEPTH_TEST);
            GL11.glTranslatef(posX, posY, posZ);
            GL11.glScalef((float) -scale, (float) scale, (float)
                    scale); // Not entirely sure why mobs are flipped but this is how vanilla GUIs fix it so...
    ...
    public static void drawHighlightedSplitString(
        ...
        List<String> noFormat =
                splitStringWithoutFormat(string, width, renderer); // Needed for accurate highlight index positions
    ...
    public static List<String> splitStringWithoutFormat(String str, int wrapWidth, FontRenderer font) {
        ...
        while (true) {
            ...
            if (temp.length() <= i) {
                ...
                // replace them with invisible new line characters to preserve the index
                // count
    ...
    private static boolean isFormatSpecial(char p_78270_0_) {
        return p_78270_0_ >= 107 && p_78270_0_ <= 111
                || p_78270_0_ >= 75 && p_78270_0_ <= 79
                || p_78270_0_ == 114
                || p_78270_0_ == 82;
    }
    ...
    public static int getStringWidth(String text, FontRenderer font) {
        ...
        for (int j = 0; j < text.length(); ++j) {
            ...
            if (k < 0 && j < text.length() - 1) // k should only be negative when the section sign has been used!
            {
                ...
                if (c0 != 'l' && c0 != 'L') {
                    int ci = "0123456789abcdefklmnor"
                            .indexOf(String.valueOf(c0).toLowerCase(Locale.ROOT).charAt(0));
