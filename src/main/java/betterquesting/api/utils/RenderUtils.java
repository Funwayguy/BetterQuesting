...
public class RenderUtils {
  ...
  public static void RenderEntity(float posX, float posY, float posZ, int scale, float rotation, float pitch,
    ...
    try {
      ...
      GlStateManager.scale((float) -scale, (float) scale,
                           (float) scale); // Not entirely sure why mobs are flipped but this is how vanilla GUIs fix it so...
  ...
  public static void drawHighlightedSplitString(FontRenderer renderer, String string, int x, int y, int width,
    ...
    drawHighlightedSplitString(renderer, string, x, y, width, color, shadow, 0,
                               splitString(string, width, renderer).size() - 1, highlightColor, highlightStart,
                               highlightEnd);
  }
  ...
  public static void drawHighlightedSplitString(FontRenderer renderer, String string, int x, int y, int width,
    ...
    if (renderer == null || string == null || string.isEmpty() || start > end) {
      return;
    }
  ...
  public static void drawHighlightedString(FontRenderer renderer, String string, int x, int y, int color,
    ...
    if (renderer == null || string == null || string.isEmpty()) {
      return;
    }
  ...
  public static void drawHoveringText(@Nonnull final ItemStack stack, List<String> textLines, int mouseX, int mouseY,
    ...
    PresetTexture.TOOLTIP_BG.getTexture()
                            .drawTexture(tooltipX - 4, tooltipY - 4, tooltipTextWidth + 8, tooltipHeight + 8, 0F, 1F);
    ...
    MinecraftForge.EVENT_BUS.post(
        new RenderTooltipEvent.PostText(stack, textLines, tooltipX, tooltipTop, font, tooltipTextWidth, tooltipHeight));
