...
public class PlaceholderConverter {
    ...
    public static BigItemStack convertItem(
            Item item, String name, int count, int damage, String oreDict, NBTTagCompound nbt) {
        if (item == null) {
            ...
            if (nbt != null) {
                ...
                if (restored != null) {
                    BigItemStack stack = new BigItemStack(
                                    restored, count, nbt.hasKey("orig_meta") ? nbt.getInteger("orig_meta") : damage)
                            .setOreDict(oreDict);
