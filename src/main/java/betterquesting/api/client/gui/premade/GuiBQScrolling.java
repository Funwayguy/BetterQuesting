package betterquesting.api.client.gui.premade;
...
public abstract class GuiBQScrolling
{
	...
	public GuiBQScrolling(int x, int y, int width, int height, int entryHeight)
	{
	...
	public void drawScreen(int mouseX, int mouseY, float p_22243_3_)
	{
		...
		RenderUtils.DrawLine(left, top, right, top, 1F, ExpansionAPI.INSTANCE.getThemeRegistry().getCurrentTheme().getTextColor());
		RenderUtils.DrawLine(left, bottom, right, bottom, 1F, ExpansionAPI.INSTANCE.getThemeRegistry().getCurrentTheme().getTextColor());
		...
		if(var19 > 0)
		{
			...
			client.renderEngine.bindTexture(ExpansionAPI.INSTANCE.getThemeRegistry().getCurrentTheme().getGuiTexture());
