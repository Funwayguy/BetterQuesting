    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (stack.getItemDamage() != 0) {
            ...
            if (lives >= QuestSettings.INSTANCE.getProperty(NativeProps.LIVES_MAX)) {
                if (!world.isRemote) {
                    player.addChatComponentMessage(new ChatComponentText(EnumChatFormatting.RED.toString())
            ...
            if (!world.isRemote) {
                ...
                player.addChatComponentMessage(new ChatComponentTranslation(
                        "betterquesting.gui.remaining_lives", EnumChatFormatting.YELLOW.toString() + (lives + 1)));
            }
