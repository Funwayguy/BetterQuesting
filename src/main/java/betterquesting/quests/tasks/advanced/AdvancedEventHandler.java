...
public class AdvancedEventHandler
{
	...
	public void onBlockBreak(BlockEvent.BreakEvent event)
	{
		...
		for(Entry<AdvancedTaskBase,QuestInstance> set : GetAdvancedTasks(player.getUniqueID()).entrySet())
		{
			set.getKey().onBlockBreak(set.getValue(), player, event.state, event.pos);
		}
	}
	...
	public void onBlockPlace(BlockEvent.PlaceEvent event)
	{
		...
		for(Entry<AdvancedTaskBase,QuestInstance> set : GetAdvancedTasks(player.getUniqueID()).entrySet())
		{
			set.getKey().onBlockPlace(set.getValue(), player, event.state, event.pos);
		}
	}
	...
	public void onBlockInteract(PlayerInteractEvent event)
	{
		...
		if(event.action == PlayerInteractEvent.Action.LEFT_CLICK_BLOCK)
		{
			for(Entry<AdvancedTaskBase,QuestInstance> set : GetAdvancedTasks(player.getUniqueID()).entrySet())
			{
				set.getKey().onBlockInteract(set.getValue(), player, event.world.getBlockState(event.pos), event.pos, true);
			}
		} else if(event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK || event.action == PlayerInteractEvent.Action.RIGHT_CLICK_AIR)
		{
			for(Entry<AdvancedTaskBase,QuestInstance> set : GetAdvancedTasks(player.getUniqueID()).entrySet())
			{
				set.getKey().onBlockInteract(set.getValue(), player, event.world.getBlockState(event.pos), event.pos, false);
			}
		}
	}
	...
	public void onNotePlayed(NoteBlockEvent.Play event) // Notifies everyone in a 64 block range about a played note
	{
		...
		for(EntityPlayer player : pList)
		{
			for(Entry<AdvancedTaskBase,QuestInstance> set : GetAdvancedTasks(player.getUniqueID()).entrySet())
			{
				set.getKey().onNotePlayed(set.getValue(), event.world, event.pos, event.instrument, event.getNote(), event.getOctave());
			}
		}
	}
