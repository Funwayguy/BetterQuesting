    ...
    public List<String> autoComplete(MinecraftServer server, ICommandSender sender, String[] args) {
        ...
        if (args.length == 4 && (args[1].equalsIgnoreCase("add") || args[1].equalsIgnoreCase("set"))) {
            return CommandBase.getListOfStringsMatchingLastWord(
                    args, NameCache.INSTANCE.getAllNames().toArray(new String[0]));
        ...
        if (action.equalsIgnoreCase("set")) {
            ...
            if (playerID != null) {
                ...
                sender.addChatMessage(
                        new ChatComponentTranslation("betterquesting.cmd.lives.set_player", pName, value));
            } else {
                for (EntityPlayerMP p : (List<EntityPlayerMP>) server.getConfigurationManager().playerEntityList) {
            ...
            if (playerID != null) {
                ...
                if (value >= 0) {
                    ...
                    sender.addChatMessage(new ChatComponentTranslation(
                            "betterquesting.cmd.lives.remove_player", Math.abs(value), pName, lives));
                }
            } else {
                for (EntityPlayerMP p : (List<EntityPlayerMP>) server.getConfigurationManager().playerEntityList) {
                    UUID uuid = QuestingAPI.getQuestingUUID(p);
