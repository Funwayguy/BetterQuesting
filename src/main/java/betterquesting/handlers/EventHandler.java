...
public class EventHandler {
  ...
  private static void postPresetNotice(IQuest quest, EntityPlayer player, int preset) {
    ...
    NetNotices.sendNotice(
        quest.getProperty(NativeProps.GLOBAL) ? null : new EntityPlayerMP[] { (EntityPlayerMP) player }, icon, mainText,
        subText, sound);
  }
    ...
    while (!opQueue.isEmpty()) {
      ...
      if (playerMP != null && NameCache.INSTANCE.updateName(playerMP)) {
        ...
        if (party != null) {
          ...
          NetNameSync.sendNames(new EntityPlayerMP[] { playerMP }, new UUID[] { QuestingAPI.getQuestingUUID(playerMP) },
                                null);
        }
      }
    }
