...
public class TileSubmitStation extends TileEntity implements IFluidHandler, ISidedInventory {
    ...
    private static final int[] SLOTS_FOR_FACE = new int[] {SLOT_INPUT, SLOT_OUTPUT};
    ...
    public boolean isUseableByPlayer(EntityPlayer player) {
        return (owner == null || player.getUniqueID().equals(owner))
                && player.getDistanceSq(this.xCoord, this.yCoord, this.zCoord) < 256;
    }
        ...
        if (wtt % 10 == 0 && owner != null) {
            ...
            if (q != null && t != null && itemStack[SLOT_INPUT] != null && itemStack[SLOT_OUTPUT] == null) {
                ...
                if (t.canAcceptItem(owner, getQuest(), inStack)) {
                    itemStack[SLOT_INPUT] = t.submitItem(
                            owner,
                            getQuest(),
                            inStack); // Even if this returns an invalid item for submission it will be moved next pass
    ...
    public void writeToNBT(NBTTagCompound tags) {
        ...
        tags.setTag(
                "input",
                itemStack[SLOT_INPUT] != null
                        ? itemStack[SLOT_INPUT].writeToNBT(new NBTTagCompound())
                        : new NBTTagCompound());
        tags.setTag(
                "output",
                itemStack[SLOT_OUTPUT] != null
                        ? itemStack[SLOT_OUTPUT].writeToNBT(new NBTTagCompound())
                        : new NBTTagCompound());
    }
