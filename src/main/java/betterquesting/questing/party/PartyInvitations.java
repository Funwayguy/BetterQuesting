...
public class PartyInvitations implements INBTPartial<NBTTagList, UUID> {
    ...
    public synchronized void cleanExpired() {
        ...
        while (iterA.hasNext()) {
            ...
            Iterator<Entry<Integer, Long>> iterB =
                    userInvites.getValue().entrySet().iterator();
            while (iterB.hasNext()) {
            ...
            if (player != null && revoked.size() >= 0) {
                ...
                NetInviteSync.sendRevoked(
                        player, revAry); // Normally I avoid including networking calls into the database...
            }
    ...
    @Override
    public synchronized NBTTagList writeToNBT(
            NBTTagList nbt,
            @Nullable List<UUID> subset) // Don't bother saving this to disk. We do need to send packets though
            {
        if (subset != null) {
            subset.forEach((uuid) -> {
