...
public class GuiQuestHelp extends GuiScreenCanvas {
  ...
  public void initPanel() {
    ...
    CanvasScrolling cvTopics =
        new CanvasScrolling(new GuiTransform(GuiAlign.LEFT_EDGE, new GuiPadding(16, 32, -108, 24), 0));
    cvBackground.addPanel(cvTopics);

    PanelVScrollBar scTopic =
        new PanelVScrollBar(new GuiTransform(GuiAlign.LEFT_EDGE, new GuiPadding(108, 32, -116, 24), 0));
    cvBackground.addPanel(scTopic);
    cvTopics.setScrollDriverY(scTopic);

    CanvasScrolling cvDesc =
        new CanvasScrolling(new GuiTransform(GuiAlign.FULL_BOX, new GuiPadding(124, 48, 24, 24), 0)).enableBlocking(
            false);
    cvBackground.addPanel(cvDesc);

    PanelVScrollBar scDesc =
    ...
    PanelTextBox txtTitle =
        new PanelTextBox(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(124, 36, 16, -48), 0), "").setAlignment(1);
    ...
    for (int i = 0; i < topics.length; i++) {
      ...
      cvTopics.addPanel(
          new PanelButtonStorage<>(new GuiRectangle(0, i * 16, width, 16, 0), -1, ht.getTitle(), ht).setCallback(
              value -> {
