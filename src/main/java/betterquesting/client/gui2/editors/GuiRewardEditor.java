...
public class GuiRewardEditor extends GuiScreenCanvas implements IPEventListener, IVolatileScreen, INeedsRefresh {
    ...
    public void initPanel() {
        ...
        PanelTextBox panTxt = new PanelTextBox(
                        ...
                        QuestTranslation.translate("betterquesting.title.edit_rewards"))
                .setAlignment(1);
        ...
        CanvasSearch<IFactoryData<IReward, NBTTagCompound>, IFactoryData<IReward, NBTTagCompound>> cvRegSearch =
                new CanvasSearch<IFactoryData<IReward, NBTTagCompound>, IFactoryData<IReward, NBTTagCompound>>(
                    ...
                    protected Iterator<IFactoryData<IReward, NBTTagCompound>> getIterator() {
                        ...
                        list.sort(Comparator.comparing(
                                o -> o.getRegistryName().toString().toLowerCase()));
                        return list.iterator();
                    }

                    @Override
                    protected void queryMatches(
                            IFactoryData<IReward, NBTTagCompound> value,
                            String query,
                    ...
                    protected boolean addResult(
                            IFactoryData<IReward, NBTTagCompound> entry, int index, int cachedWidth) {
                        this.addPanel(new PanelButtonStorage<>(
        ...
        PanelVScrollBar scReg =
                new PanelVScrollBar(new GuiTransform(GuiAlign.RIGHT_EDGE, new GuiPadding(-24, 48, 16, 32), 0));
    ...
    private void onButtonPress(PEventButton event) {
        ...
        } else if (btn.getButtonID() == 1 && btn instanceof PanelButtonStorage) // Add
        {
            IFactoryData<IReward, NBTTagCompound> fact =
                    ((PanelButtonStorage<IFactoryData<IReward, NBTTagCompound>>) btn).getStoredValue();
    ...
    private void refreshRewards() {
        ...
        for (int i = 0; i < dbRew.size(); i++) {
            ...
            qrList.addPanel(new PanelButtonStorage<>(
