...
public class GuiQuestLinesEditor extends GuiScreenCanvas implements IPEventListener, IVolatileScreen, INeedsRefresh {
  ...
  public void initPanel() {
    ...
    PanelTextBox panTxt = new PanelTextBox(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(0, 16, 0, -32), 0),
                                           QuestTranslation.translate("betterquesting.title.edit_line1")).setAlignment(
        1);
    ...
    PanelVScrollBar scList =
        new PanelVScrollBar(new GuiTransform(new Vector4f(0.5F, 0F, 0.5F, 1F), new GuiPadding(-16, 32, 8, 48), 0));
    cvBackground.addPanel(scList);
    lineList.setScrollDriverY(scList);

    PanelButton btnAdd =
        new PanelButton(new GuiTransform(new Vector4f(0F, 1F, 0.25F, 1F), new GuiPadding(16, -40, 0, 24), 0), 1,
                        QuestTranslation.translate("betterquesting.btn.new"));
    cvBackground.addPanel(btnAdd);

    PanelButton btnImport =
        new PanelButton(new GuiTransform(new Vector4f(0.25F, 1F, 0.5F, 1F), new GuiPadding(0, -40, 16, 24), 0), 2,
    ...
    btnVis = new PanelButton(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(0, 96, 0, -112), 0), -1,
                             ...
                             (selected == null ? EnumQuestVisibility.NORMAL
                                               : selected.getProperty(NativeProps.VISIBILITY))) {
      @Override
      public void onButtonClick() {
        if (selected == null) {
        ...
        setText(QuestTranslation.translate("betterquesting.btn.show") + ": " + vis);
      }
    };
    btnVis.setActive(selected != null);
    cvRight.addPanel(btnVis);

    PanelButton btnManage = new PanelButton(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(0, 112, 0, -128), 0), 3,
    ...
    PanelButton btnTextEditor =
        new PanelButton(new GuiTransform(GuiAlign.TOP_RIGHT, new GuiPadding(-16, 48, 0, -64), 0), 8, "Aa");
  ...
  private void onButtonPress(PEventButton event) {
    ...
    } else if (btn.getButtonID() == 5 && btn instanceof PanelButtonStorage) // Select Quest
    {
      ...
      btnVis.setActive(true);
      btnVis.setText(
          QuestTranslation.translate("betterquesting.btn.show") + ": " + selected.getProperty(NativeProps.VISIBILITY));
  ...
  private void reloadList() {
    ...
    for (DBEntry<IQuestLine> entry : QuestLineDatabase.INSTANCE.getSortedEntries()) {
      ...
      PanelButtonStorage<DBEntry<IQuestLine>> tmp =
          new PanelButtonStorage<>(new GuiRectangle(0, i * 16, w - 32, 16, 0), 5,
                                   QuestTranslation.translate(ql.getUnlocalisedName()), entry);
      tmp.setActive(entry.getID() != selID);
      lineList.addPanel(tmp);
      lineList.addPanel(new PanelButtonStorage<>(new GuiRectangle(w - 32, i * 16, 16, 16, 0), 6, "", entry).setIcon(
          PresetIcon.ICON_TRASH.getTexture()));
      PanelButton btnUp = new PanelButtonStorage<>(new GuiRectangle(w - 16, i * 16, 16, 16, 0), 7, "", entry).setIcon(
