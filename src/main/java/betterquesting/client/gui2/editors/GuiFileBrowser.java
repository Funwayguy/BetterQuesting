...
public class GuiFileBrowser extends GuiScreenCanvas implements IPEventListener {
    ...
    public void initPanel() {
        ...
        txtTitle = new PanelTextBox(
                        ...
                        curDirectory == null ? "*" : curDirectory.getAbsolutePath())
                .setAlignment(1);
        ...
        PanelTextBox txtDb = new PanelTextBox(
                        new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(0, 0, 0, -16), 0),
                        QuestTranslation.translate("betterquesting.gui.folder"))
                .setAlignment(1);
        cvRight.addPanel(txtDb);

        PanelTextField<String> searchBox = new PanelTextField<>(
                new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(16, 16, 8, -32), 0), "", FieldFilterString.INSTANCE);
        searchBox.setWatermark("Search...");
        cvRight.addPanel(searchBox);

        cvDirectory =
                new CanvasFileDirectory(
                        new GuiTransform(GuiAlign.FULL_BOX, new GuiPadding(0, 32, 8, 0), 0), curDirectory, filter) {
                    @Override
                    protected boolean addResult(File entry, int index, int width) {
                        if (!entry.isDirectory()) {
                            PanelButtonStorage<File> btnAdd =
                                    new PanelButtonStorage<>(new GuiRectangle(0, index * 16, 16, 16, 0), -1, "", entry);
                            ...
                            // Keeps the scrolling region's left side from auto-cropping when no files are present to
                            // select.
                        ...
                        PanelGeneric pnIco = new PanelGeneric(
                                ...
                                entry.isDirectory()
                                        ? PresetIcon.ICON_FOLDER_OPEN.getTexture()
    ...
    private void refreshSelected() {
        ...
        for (int i = 0; i < selList.size(); i++) {
            ...
            PanelButtonStorage<File> btnFile =
                    new PanelButtonStorage<>(new GuiRectangle(width - 16, i * 16, 16, 16, 0), -1, "", f);
