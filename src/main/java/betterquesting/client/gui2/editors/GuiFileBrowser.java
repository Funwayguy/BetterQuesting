...
public class GuiFileBrowser extends GuiScreenCanvas implements IPEventListener {
  ...
  public void initPanel() {
    ...
    PanelTextBox txtQuest = new PanelTextBox(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(0, 0, 0, -16), 0),
                                             QuestTranslation.translate("betterquesting.gui.selection")).setAlignment(
        1);
    ...
    PanelTextField<String> searchBox =
        new PanelTextField<>(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(16, 16, 24, -32), 0), "",
                             FieldFilterString.INSTANCE);
    searchBox.setWatermark("Search...");
    cvRight.addPanel(searchBox);

    cvDirectory =
        new CanvasFileDirectory(new GuiTransform(GuiAlign.FULL_BOX, new GuiPadding(0, 32, 8, 0), 0), curDirectory,
                                filter) {
          @Override
