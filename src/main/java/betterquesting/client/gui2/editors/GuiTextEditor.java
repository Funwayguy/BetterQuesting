...
public class GuiTextEditor extends GuiScreenCanvas implements IPEventListener, IVolatileScreen {
    ...
    public void initPanel() {
        ...
        PanelTextBox txTitle = new PanelTextBox(
                        ...
                        QuestTranslation.translate("betterquesting.title.edit_text"))
                .setAlignment(1);
        ...
        CanvasScrolling cvFormatList =
                new CanvasScrolling(new GuiTransform(GuiAlign.LEFT_EDGE, new GuiPadding(16, 32, -116, 32), 0));
        cvBackground.addPanel(cvFormatList);

        EnumChatFormatting[] tfValues = EnumChatFormatting.values();
        for (int i = 0; i < tfValues.length; i++) {
            cvFormatList.addPanel(new PanelButtonStorage<>(
                    new GuiRectangle(0, i * 16, 100, 16), 1, tfValues[i].getFriendlyName(), tfValues[i].toString()));
        }

        PanelVScrollBar scFormatScroll =
