...
public class GuiHome extends GuiScreenCanvas {
  ...
  public void initPanel() {
    ...
    CanvasTextured splashTitle = new CanvasTextured(
        new GuiTransform(new Vector4f(ancX, ancY, ancX, ancY), new GuiPadding(offX, offY, -256 - offX, -128 - offY), 0),
        homeSplashTitle);
    splashCan.addPanel(splashTitle);

    PanelButton btnExit =
        new PanelButton(new GuiTransform(new Vector4f(0F, 1F, 0.25F, 1F), new GuiPadding(0, -32, 0, 0), 0), 0,
                        QuestTranslation.translate("betterquesting.home.exit"));
    inCan.addPanel(btnExit);

    PanelButton btnQuests =
        new PanelButton(new GuiTransform(new Vector4f(0.25F, 1F, 0.5F, 1F), new GuiPadding(0, -32, 0, 0), 0), 1,
                        QuestTranslation.translate("betterquesting.home.quests"));
    inCan.addPanel(btnQuests);
    PanelButton btnParty =
        new PanelButton(new GuiTransform(new Vector4f(0.5F, 1F, 0.75F, 1F), new GuiPadding(0, -32, 0, 0), 0), 2,
    ...
    PanelButton btnTheme =
        new PanelButton(new GuiTransform(new Vector4f(0.75F, 1F, 1F, 1F), new GuiPadding(0, -32, 0, 0), 0), 3,
    ...
    if (QuestingAPI.getAPI(ApiReference.SETTINGS).canUserEdit(mc.player)) {
      PanelButton btnEdit =
          new PanelButton(new GuiTransform(GuiAlign.TOP_LEFT, new GuiPadding(0, 0, -16, -16), 0), 4, "").setIcon(
    ...
    if (Minecraft.getMinecraft().isIntegratedServerRunning() && SaveLoadHandler.INSTANCE.hasUpdate()) {
      ...
      tstBtn.setTooltip(
          Collections.singletonList(QuestTranslation.translateTrimmed("betterquesting.tooltip.update_quests", true)));
