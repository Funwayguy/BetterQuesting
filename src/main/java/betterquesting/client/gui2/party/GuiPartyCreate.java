...
public class GuiPartyCreate extends GuiScreenCanvas implements IPEventListener, INeedsRefresh {
    ...
    public void initPanel() {
        ...
        PanelTextBox txTitle = new PanelTextBox(
                        ...
                        QuestTranslation.translate("betterquesting.title.party_none"))
                .setAlignment(1);
        txTitle.setColor(PresetColor.TEXT_HEADER.getColor());
        cvBackground.addPanel(txTitle);

        CanvasEmpty cvLeftHalf =
                new CanvasEmpty(new GuiTransform(GuiAlign.HALF_LEFT, new GuiPadding(16, 32, 8, 32), 0));
        cvBackground.addPanel(cvLeftHalf);

        PanelPlayerPortrait pnPortrait = new PanelPlayerPortrait(
                        new GuiTransform(GuiAlign.TOP_CENTER, -32, 0, 64, 64, 0), mc.thePlayer)
                .setDepth(-16F);
        cvLeftHalf.addPanel(pnPortrait);

        cvLeftHalf.addPanel(new PanelGeneric(
        ...
        PanelTextBox txInvite = new PanelTextBox(
                        ...
                        QuestTranslation.translate("betterquesting.gui.party_invites"))
                .setAlignment(1);
    ...
    private void refreshInvites() {
        ...
        for (int i = 0; i < invites.size(); i++) {
            ...
            ValueFuncIO<Float> expFunc =
                    new ValueFuncIO<>(() -> (float) ((exp - System.currentTimeMillis()) / 300000D));
            flExpiry.setFillDriver(expFunc);
            flExpiry.setFillColor(new GuiColorTransition(
                            new GuiColorStatic(0xFF00FF00), new GuiColorPulse(0xFFFF0000, 0xFFC00000, 1F, 0F))
                    .setupBlending(false, 0.2F)
