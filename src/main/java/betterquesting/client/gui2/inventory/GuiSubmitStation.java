...
public class GuiSubmitStation extends GuiContainerCanvas implements INeedsRefresh {
  ...
  public void initPanel() {
    ...
    PanelTextBox txtTitle = new PanelTextBox(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(0, 16, 0, -32), 0),
                                             QuestTranslation.translate(
                                                 "betterquesting.title.submit_station")).setAlignment(1);
    ...
    cvBackground.addPanel(new PanelButton(new GuiTransform(GuiAlign.BOTTOM_CENTER, -100, -16, 200, 16, 0), -1,
                                          QuestTranslation.translate("gui.done")).setClickAction(
    ...
    btnSet = new PanelButton(new GuiTransform(new Vector4f(0.75F, 0F, 0.75F, 0F), -16, 64, 16, 16, 0), -1, "") {
      @Override
      public void onButtonClick() {
        DBEntry<IQuest> selQuestEntry = quests.get(selQuest);
        DBEntry<ITask> selTaskEntry = tasks.get(selTask);
        tile.setupTask(QuestingAPI.getQuestingUUID(mc.player), selQuestEntry.getID(), selTaskEntry.getID());
        NetStationEdit.setupStation(tile.getPos(),
                                    selQuestEntry.getID(),
                                    selTaskEntry.getID(),
                                    (byte) ssContainer.windowId);
        refreshTaskPanel();
      }
    };
    ...
    btnRem = new PanelButton(new GuiTransform(new Vector4f(0.75F, 0F, 0.75F, 0F), 0, 64, 16, 16, 0), -1, "") {
      ...
      public void onButtonClick() {
        tile.reset();
        NetStationEdit.resetStation(tile.getPos(), (byte) ssContainer.windowId);
        refreshTaskPanel();
      }
    };
    ...
    txtQstTitle =
        new PanelTextBox(new GuiTransform(new Vector4f(0.5F, 0F, 1F, 0F), new GuiPadding(24, 36, 32, -48), 0), "");
    txtQstTitle.setColor(PresetColor.TEXT_MAIN.getColor()).setAlignment(1);
    cvBackground.addPanel(txtQstTitle);

    txtTskTitle =
        new PanelTextBox(new GuiTransform(new Vector4f(0.5F, 0F, 1F, 0F), new GuiPadding(24, 52, 32, -64), 0), "");
  ...
  private void filterQuests() {
    quests.removeIf(q -> {
      ...
      for (DBEntry<ITask> tsk : q.getValue().getTasks().getEntries()) {
        if (tsk.getValue() instanceof IItemTask ||
            tsk.getValue() instanceof IFluidTask ||
            tsk.getValue() instanceof IEnergyTask) {
          shouldDelete = false;
          break;
        }
      }
  ...
  private void refreshTaskPanel() {
    ...
    ITask curTask = tasks.get(selTask).getValue();
    txtTskTitle.setText(QuestTranslation.translate(curTask.getUnlocalisedName()));
    btnSet.setActive(!tile.isSetup() && (curTask instanceof IItemTask ||
                                         curTask instanceof IFluidTask ||
