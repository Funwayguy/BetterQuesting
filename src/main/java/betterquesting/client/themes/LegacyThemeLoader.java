...
public class LegacyThemeLoader {
    ...
    protected IGuiTheme loadTheme(JsonObject json, String domain) {
        ...
        int tColor =
                JsonHelper.GetNumber(json, "textColor", Color.BLACK.getRGB()).intValue();

        JsonObject jl = JsonHelper.GetObject(json, "lineColor");
        int lLocked = JsonHelper.GetNumber(jl, "locked", new Color(0.75F, 0F, 0F).getRGB())
                .intValue();
        int lPending =
                JsonHelper.GetNumber(jl, "pending", Color.YELLOW.getRGB()).intValue();
        int lComplete =
                JsonHelper.GetNumber(jl, "complete", Color.GREEN.getRGB()).intValue();
        ...
        int iUnclaimed = JsonHelper.GetNumber(ji, "unclaimed", new Color(0F, 1F, 1F).getRGB())
                .intValue();
        int iComplete =
                JsonHelper.GetNumber(ji, "complete", Color.GREEN.getRGB()).intValue();
