  ...
  public ThemeRegistry() {
    ...
    setDefaultGui(PresetGUIs.EDIT_NBT, arg ->
    {
      if (arg.value instanceof NBTTagCompound) {
        ...
        return new GuiNbtEditor(arg.parent, (NBTTagCompound) arg.value, (ICallback<NBTTagCompound>) arg.callback);
      } else if (arg.value instanceof NBTTagList) {
        //noinspection unchecked
        return new GuiNbtEditor(arg.parent, (NBTTagList) arg.value, (ICallback<NBTTagList>) arg.callback);
      } else {
        return null;
      }
    });
    ...
    setDefaultGui(PresetGUIs.FILE_EXPLORE,
                  arg -> new GuiFileBrowser(arg.parent, arg.callback, arg.root, arg.filter).allowMultiSelect(
                      arg.multiSelect));
  }
      ...
      for (IResource iresource : list) {
        try (InputStreamReader isr = new InputStreamReader(iresource.getInputStream(), StandardCharsets.UTF_8)) {
          ...
          for (int i = 0; i < jAry.size(); i++) {
            ...
            ResourceLocation parentID = !jThm.has("themeParent") ? null : new ResourceLocation(
                JsonHelper.GetString(jThm, "themeParent", "minecraft:null"));
            ...
            try {
              ...
              BetterQuesting.logger.error("Failed to load theme entry " + i + " in " + iresource.getResourceLocation(),
                                          e);
              continue;
            }
