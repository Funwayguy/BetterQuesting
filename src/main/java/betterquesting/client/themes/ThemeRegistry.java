...
public class ThemeRegistry implements IThemeRegistry {
    ...
    private static final IGuiTexture NULL_TEXTURE =
            new SlicedTexture(PresetTexture.TX_NULL, new GuiRectangle(0, 0, 32, 32), new GuiPadding(8, 8, 8, 8));
    ...
    private void setTheme(IGuiTheme theme, ResourceLocation id) {
        ...
        if (ConfigHandler.config != null) {
            ConfigHandler.config
                    .get(Configuration.CATEGORY_GENERAL, "Theme", "")
                    ...
                    for (int i = 0; i < jAry.size(); i++) {
                        ...
                        if (!(je instanceof JsonObject)) {
                            BetterQuesting.logger.log(
                                    Level.WARN, "Invalid theme entry at index " + i + " in " + domain);
                            continue;
                        }
                        ...
                        ResourceLocation parentID = !jThm.has("themeParent")
                                ? null
                        ...
                        for (Entry<String, JsonElement> entry : jsonTextureRoot.entrySet()) {
                            ...
                            ResourceLocation typeID =
                                    new ResourceLocation(JsonHelper.GetString(joTex, "textureType", ""));
                            IFactoryData<IGuiTexture, JsonObject> tFact =
                                    ResourceRegistry.INSTANCE.getTexReg().getFactory(typeID);
                            ...
                            if (gTex == null) {
                                BetterQuesting.logger.error("Failed to load texture type " + typeID + " for theme "
                                        + themeName + " in " + domain);
                                continue;
                            }
                        ...
                        for (Entry<String, JsonElement> entry : jsonColourRoot.entrySet()) {
                            ...
                            ResourceLocation typeID =
                                    new ResourceLocation(JsonHelper.GetString(joCol, "colorType", ""));
                            IFactoryData<IGuiColor, JsonObject> cFact =
                                    ResourceRegistry.INSTANCE.getColorReg().getFactory(typeID);
                            ...
                            if (gCol == null) {
                                BetterQuesting.logger.error("Failed to load color type " + typeID + " for theme "
                                        + themeName + " in " + domain);
                                continue;
                            }
                        ...
                        for (Entry<String, JsonElement> entry : jsonLinesRoot.entrySet()) {
                            ...
                            ResourceLocation typeID =
                                    new ResourceLocation(JsonHelper.GetString(joLine, "lineType", ""));
                            IFactoryData<IGuiLine, JsonObject> lFact =
                                    ResourceRegistry.INSTANCE.getLineReg().getFactory(typeID);

                            if (lFact == null) {
                                BetterQuesting.logger.error(
                                        "Unknown line type " + typeID + " for theme " + themeName + " in " + domain);
                                continue;
                            }
                            ...
                            if (gLine == null) {
                                BetterQuesting.logger.error("Failed to load line type " + typeID + " for theme "
                                        + themeName + " in " + domain);
                                continue;
                            }
