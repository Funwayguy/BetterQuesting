    ...
    public void disableTool() {
        if (grabList.size() > 0) {
            for (GrabEntry grab : grabList) {
                IQuestLineEntry qle =
                        gui.getQuestLine().getValue(grab.btn.getStoredValue().getID());
            ...
            if (PanelToolController.selected.size() > 0) {
                ...
                for (PanelButtonQuest btn : PanelToolController.selected) {
                    if (first) {
                        ...
                        scaleBounds.w =
                                Math.max(scaleBounds.x + scaleBounds.w, btn.rect.x + btn.rect.w) - scaleBounds.x;
                        scaleBounds.h =
                                Math.max(scaleBounds.y + scaleBounds.h, btn.rect.y + btn.rect.h) - scaleBounds.y;
                    }
                }
