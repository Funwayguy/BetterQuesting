...
public class ToolboxToolCopy implements IToolboxTool {
  ...
  public List<String> getTooltip(int mx, int my) {
    return grabList.isEmpty() ? null : Collections.emptyList();
  }
      ...
      if (btnClicked != null) // Pickup the group or the single one if none are selected
      {
        if (!PanelToolController.selected.isEmpty()) {
          ...
          for (PanelButtonQuest btn : PanelToolController.selected) {
            ...
            grabList.add(
                new GrabEntry(new PanelButtonQuest(rect, -1, "", btn.getStoredValue()), rect.x - btnClicked.rect.x,
                              rect.y - btnClicked.rect.y));
          }
        } else {
          grabList.add(new GrabEntry(
              new PanelButtonQuest(new GuiRectangle(btnClicked.rect), -1, "", btnClicked.getStoredValue()), 0, 0));
        }
