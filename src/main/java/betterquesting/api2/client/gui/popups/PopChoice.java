...
public class PopChoice extends CanvasEmpty
{
    ...
    public void initPanel()
    {
        ...
        CanvasResizeable cvBox = new CanvasResizeable(new GuiTransform(new GuiAnchor(0.5F, 0.45F, 0.5F, 0.45F)), PresetTexture.PANEL_MAIN.getTexture());
        this.addPanel(cvBox);
        cvBox.lerpToRect(new GuiTransform(new GuiAnchor(0.2F, 0.3F, 0.8F, 0.6F)), 200L, true);
        ...
        for(int i = 0; i < options.length; i++)
        {
            ...
            PanelButton btn = new PanelButton(new GuiTransform(new GuiAnchor(0.5F, 0.6F, 0.5F, 0.6F), -rowW / 2 + rowX * 112, 8 + 24 * rowY, 96, 16, 0), -1, options[i]);
