...
public class PopChoice extends CanvasEmpty {
  ...
  public void initPanel() {
    ...
    cvBox.addPanel(
        new PanelTextBox(new GuiTransform(GuiAlign.FULL_BOX, new GuiPadding(8, 8, 8, 8), 0), message).setAlignment(1));
    ...
    for (int i = 0; i < options.length; i++) {
      ...
      PanelButton btn = new PanelButton(
          new GuiTransform(new Vector4f(0.5F, 0.6F, 0.5F, 0.6F), -rowW / 2 + rowX * 112, 8 + 24 * rowY, 96, 16, 0), -1,
          options[i]);
