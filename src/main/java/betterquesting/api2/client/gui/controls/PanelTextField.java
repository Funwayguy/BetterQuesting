...
public class PanelTextField<T> implements IGuiPanel {
    ...
    public void setSelectionPos(int position) {
        ...
        if (selectEnd != position) {
            ...
            if (canWrap) {
                List<String> lines = RenderUtils.splitStringWithoutFormat(
                        text, getTransform().getWidth() - 8, font);
                String lastFormat = "";
                int idx = 0;
                int y = 0;
                int x = 0;
    ...
    public void drawPanel(int mx, int my, float partialTick) {
        ...
        if (isActive && dragging && Mouse.isButtonDown(0)) {
            if (canWrap) {
                setSelectionPos(RenderUtils.getCursorPos(
                        text,
        ...
        if (text.length() <= 0) {
            if (!isFocused) {
                mc.fontRenderer.drawString(
                        watermark, bounds.getX() + 4, bounds.getY() + 4, colWatermark.getRGB(), false);
            }
        } else {
    ...
    public boolean onMouseClick(int mx, int my, int button) {
        if (transform.contains(mx, my)) {
            ...
            if (canWrap) {
                setCursorPosition(RenderUtils.getCursorPos(
                        text,
                        mx - (transform.getX() + 4) + getScrollX(),
                        my - (transform.getY() + 4) + getScrollY(),
                        transform.getWidth() - 8,
                ...
                setCursorPosition(RenderUtils.getCursorPos(
                        text, mx - (transform.getX() + 4) + getScrollX(), Minecraft.getMinecraft().fontRenderer));
            }
            dragging = true;
