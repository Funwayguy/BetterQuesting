...
public class PanelTextField<T> implements IGuiPanel
{
    ...
    public void drawPanel(int mx, int my, float partialTick)
    {
        if(isActive && dragging && Mouse.isButtonDown(0))
        {
            if(canWrap)
            {
                setSelectionPos(RenderUtils.getCursorPos(text, mx - (transform.getX() + 4) + getScrollX(), my - (transform.getY() + 4) + getScrollY(), transform.getWidth() - 8, Minecraft.getMinecraft().fontRendererObj));
            } else
            {
                setSelectionPos(RenderUtils.getCursorPos(text, mx - (transform.getX() + 4) + getScrollX(), Minecraft.getMinecraft().fontRendererObj));
            }
        } else if(dragging)
        {
            dragging = false;
        ...
        if(text.length() <= 0)
        {
            if(!isFocused)
            {
                mc.fontRendererObj.drawString(watermark, bounds.getX() + 4, bounds.getY() + 4, colWatermark.getRGB(), false);
            }
        } else
        {
            ...
            if(!canWrap)
            {
                RenderUtils.drawHighlightedString(mc.fontRendererObj, text, bounds.getX() + 4, bounds.getY() + 4, c.getRGB(), false, colHighlight.getRGB(), selectStart, selectEnd);
            } else
            {
                RenderUtils.drawHighlightedSplitString(mc.fontRendererObj, text, bounds.getX() + 4, bounds.getY() + 4, bounds.getWidth() - 8, c.getRGB(), false, colHighlight.getRGB(), selectStart, selectEnd);
            }
        }
    ...
    public boolean onMouseClick(int mx, int my, int button)
    {
        if(transform.contains(mx, my))
        {
            ...
            if(canWrap)
            {
                setCursorPosition(RenderUtils.getCursorPos(text, mx - (transform.getX() + 4) + getScrollX(), my - (transform.getY() + 4) + getScrollY(), transform.getWidth() - 8, Minecraft.getMinecraft().fontRendererObj));
            } else
            {
                setCursorPosition(RenderUtils.getCursorPos(text, mx - (transform.getX() + 4) + getScrollX(), Minecraft.getMinecraft().fontRendererObj));
            }
            dragging = true;
