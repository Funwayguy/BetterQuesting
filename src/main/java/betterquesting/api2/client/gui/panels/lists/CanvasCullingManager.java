...
public class CanvasCullingManager {
    ...
    private final List<IGuiPanel> cachedPanels =
            new CopyOnWriteArrayList<>(); // The last updated list of visible panels
    ...
    private static class RegionInfo {
        ...
        private final GuiRectangle
                rect; // Needs to be updated with the min-max bounds (doesn't actually conform to any grid)
        ...
        private void refreshBounds() {
            ...
            for (IGuiPanel pan : panels) {
                if (!set) {
                    ...
                    maxX = Math.max(
                            maxX, pan.getTransform().getX() + pan.getTransform().getWidth());
                    maxY = Math.max(
                            maxY, pan.getTransform().getY() + pan.getTransform().getHeight());
                }
            }
