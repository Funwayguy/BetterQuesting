    ...
    public void drawPanel(int mx, int my, float partialTick) {
        ...
        if (isDragging
                && (Mouse.isButtonDown(0)
                        || Mouse.isButtonDown(2))) // Extra fallback incase something used the usual release event
        {
        ...
        if (lsz != zs) {
            ...
            if ((zoomIn && !BQ_Settings.zoomInToCursor) || (!zoomIn && !BQ_Settings.zoomOutToCursor)) {
                float change = zs
                        / lsz; // This could probably crash if someone allowed the zoom driver to go to zero. Not really
                // my fault to be fair
                ...
                this.refreshScrollBounds(); // NOTE: This runs updatePanelPcroll() too. Hence why the math above is
                // done first before the scroll bounds are changed
                ...
                this.refreshScrollBounds(); // NOTE: This runs updatePanelScroll() too. Hence why the math above is
                // done first before the scroll bounds are changed
