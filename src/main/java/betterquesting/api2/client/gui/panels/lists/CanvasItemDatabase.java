        ...
        for (ItemStack subItem : subList) {
          try {
            ...
            BetterQuesting.logger.error(
                ...
                item.getRegistryName() + "\" (" + item.getClass().getName() + ").\nNBT: " +
                subItem.writeToNBT(new NBTTagCompound()), e);
          }
        }
      }
    } catch (Exception e) {
      BetterQuesting.logger.error(
          "An error occured while searching item \"" + item.getRegistryName() + "\" (" + item.getClass().getName() +
          ")", e);
    }
  }
