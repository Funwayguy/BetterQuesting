...
public class CanvasItemDatabase extends CanvasSearch<ItemStack, Item> {
    ...
    protected void queryMatches(Item item, String query, final ArrayDeque<ItemStack> results) {
        ...
        String regName = Item.itemRegistry.getNameForObject(
                item); // This is gonna be really slow but... meh. It's rate limited in the parent class
        ...
        try {
            ...
            if (regName.toLowerCase().contains(query)
                    || item.getUnlocalizedName().toLowerCase().contains(query)
            ...
            subList.parallelStream().forEach((subItem) -> {
                try {
                    if (subItem.getUnlocalizedName().toLowerCase().contains(query)
                            || subItem.getDisplayName().toLowerCase().contains(query)) {
                        results.add(subItem);
                    } else if (Arrays.stream(OreDictionary.getOreIDs(subItem))
                            .anyMatch((id) ->
                                    OreDictionary.getOreName(id).toLowerCase().contains(query))) {
                        results.add(subItem);
                    }
                } catch (Exception e) {
                    BetterQuesting.logger.error(
                            "An error occured while searching itemstack " + subItem.toString() + " from item \""
                                    + regName + "\" (" + item.getClass().getName() + ").\nNBT: "
