                ...
                if (parBtn != null) {
                    ...
                    switch (type) {
                        ...
                        case IMPLICIT:
                            ...
                            predicate = (mx, my, partialTicks) ->
                                    questBtns.get(req.getID()).rect.contains(mx, my)
                                            || questBtns.get(quest.getID()).rect.contains(mx, my)
                                            || Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)
                                            || Keyboard.isKeyDown(Keyboard.KEY_RSHIFT);
                            txLineCol = new GuiColorPulse(
                                    txLineCol, PresetColor.QUEST_LINE_IMPLICIT_MIXIN.getColor(), 2F, 0F);
                            break;
                        default:
                            // bail early
                            continue;
                    }
                    this.addPanel(new PanelLine(
                            parBtn.getTransform(),
