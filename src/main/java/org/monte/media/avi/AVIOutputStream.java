...
public class AVIOutputStream extends AbstractAVIStream
{
	...
	public int addVideoTrack(String fccHandler, long scale, long rate, int width, int height, int depth, int syncInterval) throws IOException
	{
		...
		VideoTrack vt = new VideoTrack(tracks.size(), typeToInt(fccHandler),//
				new Format(MediaTypeKey, MediaType.VIDEO, MimeTypeKey, MIME_AVI, EncodingKey, fccHandler, DataClassKey, byte[].class, WidthKey, width, HeightKey, height, DepthKey, depth, FixedFrameRateKey, true, FrameRateKey, new Rational(rate, scale)));
		vt.scale = scale;
		vt.rate = rate;
	...
	private void writeProlog() throws IOException
	{
		...
		for(Track tr : tracks)
		{
			...
			{
				DataChunk d = new DataChunk(c.getID(), c.getSize());
			...
			if(tr.name != null)
			{
				...
				DataChunk d = new DataChunk(STRN_ID, data.length);
				ImageOutputStream dout = d.getOutputStream();
				dout.write(data);
				d.finish();
				strlChunk.add(d);
