...
public class AVIOutputStreamOLD
{
	...
	public AVIOutputStreamOLD(File file, AVIVideoFormat format, int bitsPerPixel) throws IOException
	{
		...
		streamOffset = 0;
		videoFormat = format;
		videoFrames = new LinkedList<Sample>();
		imgDepth = bitsPerPixel;
		if(imgDepth == 4)
		{
	...
	public void writeFrame(BufferedImage image) throws IOException
	{
		...
		} else
		{
			...
			{
				throw new IllegalArgumentException("Dimensions of image[" + videoFrames.size() + "] (width=" + image.getWidth() + ", height=" + image.getHeight() + ") differs from image[0] (width=" + imgWidth + ", height=" + imgHeight);
			}
		}
	...
	public void writeFrame(InputStream in) throws IOException
	{
		...
		DataChunk videoFrameChunk = new DataChunk(videoFormat == AVIVideoFormat.RAW ? "00db" : "00dc");
		moviChunk.add(videoFrameChunk);
