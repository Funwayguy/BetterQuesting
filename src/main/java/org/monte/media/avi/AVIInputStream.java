...
public class AVIInputStream extends AbstractAVIStream
{
	...
	{
		this.in = in;
		streamOffset = in.getStreamPosition();
		in.setByteOrder(ByteOrder.LITTLE_ENDIAN);
	}
		...
		try
		{
			RIFFVisitor v = new RIFFVisitor()
			{
				...
				public boolean enteringGroup(RIFFChunk group)
				{
					...
					{
						if(mainHeader != null && (mainHeader.flags & AVIH_FLAG_HAS_INDEX) != 0 && p.getStreamOffset() == 0)
						{
	...
	private void readVideoSTRF(VideoTrack tr, byte[] data) throws IOException
	{
		...
		tr.format = new Format(MimeTypeKey, MIME_AVI, MediaTypeKey, MediaType.VIDEO, EncodingKey, tr.compression, DataClassKey, byte[].class, WidthKey, tr.width, HeightKey, tr.height, DepthKey, tr.bitCount, PixelAspectRatioKey, new Rational(1, 1), FrameRateKey, new Rational(tr.rate, tr.scale), FixedFrameRateKey, true);
	}
