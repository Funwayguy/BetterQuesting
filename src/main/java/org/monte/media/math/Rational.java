...
public class Rational extends Number
{
	...
	private Rational add(Rational that, boolean reduceFraction)
	{
		if(den == that.den)
		{
			...
			return new Rational(num + that.num, den, reduceFraction);
		}
		
		// FIXME - handle overflow
		long s = scm(den, that.den);
		Rational result = new Rational(num * (s / den) + that.num * (s / that.den), s, reduceFraction);
	...
	{
		if(abs(num) < Integer.MAX_VALUE && abs(den) < Integer.MAX_VALUE && abs(that.num) < Integer.MAX_VALUE && abs(that.den) < Integer.MAX_VALUE)
		{
			return valueOf(num * that.num, den * that.den);
		} else
		{
			return new Rational(BigInteger.valueOf(num).multiply(BigInteger.valueOf(that.num)), BigInteger.valueOf(den).multiply(BigInteger.valueOf(that.den)), true);
		}
	}
	...
	public Rational multiply(long integer)
	{
		...
		{
			return ZERO;
		} else if(den % integer == 0)
		{
			return valueOf(num, den / integer);
		} else if(abs(num) < Integer.MAX_VALUE && abs(integer) < Integer.MAX_VALUE)
		{
			return valueOf(num * integer, den);
		} else
		{
			return new Rational(BigInteger.valueOf(num).multiply(BigInteger.valueOf(integer)), BigInteger.valueOf(den), true);
		}
	}
	...
	public Rational divide(Rational that)
	{
		if(abs(num) < Integer.MAX_VALUE && abs(den) < Integer.MAX_VALUE && abs(that.num) < Integer.MAX_VALUE && abs(that.den) < Integer.MAX_VALUE)
		{
			return valueOf(num * that.den, den * that.num);
		} else
		{
			return valueOf(BigInteger.valueOf(num).multiply(BigInteger.valueOf(that.den)), BigInteger.valueOf(den).multiply(BigInteger.valueOf(that.num)), true);
		}
	}
	...
	public int compareTo(Rational that)
	{
		...
		if(abs(num) < Integer.MAX_VALUE && abs(den) < Integer.MAX_VALUE && abs(that.num) < Integer.MAX_VALUE && abs(that.den) < Integer.MAX_VALUE)
		{
			long lhs = num * that.den;
			long rhs = den * that.num;
			if(lhs < rhs)
			{
				return -1;
			} else if(lhs > rhs)
		...
		BigInteger lhs;
		BigInteger rhs;
		lhs = BigInteger.valueOf(num).multiply(BigInteger.valueOf(that.den));
		rhs = BigInteger.valueOf(den).multiply(BigInteger.valueOf(that.num));
	...
	public int hashCode()
	{
		return (int)((num ^ (num >>> 32)) ^ (den ^ (den >>> 32)));
