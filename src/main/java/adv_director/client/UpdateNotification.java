...
public class UpdateNotification
{
	...
	{
		if(!AdvDirector.proxy.isClient() || hasChecked)
		{
			return;
		}
		...
		if(AdvDirector.HASH == "CI_MOD_" + "HASH")
		{
			event.player.addChatMessage(new TextComponentString(TextFormatting.RED + "THIS COPY OF " + AdvDirector.NAME.toUpperCase() + " IS NOT FOR PUBLIC USE!"));
			return;
		}
		...
		try
		{
			...
			for(String s : data)
			{
				if(s.equalsIgnoreCase("git_branch:" + AdvDirector.BRANCH))
				{
					if(!hasLog)
					{
						hasLog = true;
			...
			{
				event.player.addChatMessage(new TextComponentString(TextFormatting.RED + "An error has occured while checking " + AdvDirector.NAME + " version!"));
				AdvDirector.logger.log(Level.ERROR, "An error has occured while checking " + AdvDirector.NAME + " version! (hasLog: " + hasLog + ", data: " + data.length + ")");
				return;
			} else
			{
			...
			boolean hasUpdate = !AdvDirector.HASH.equalsIgnoreCase(hash);
			
			if(hasUpdate)
			{
				event.player.addChatMessage(new TextComponentString(TextFormatting.RED + "Update for " + AdvDirector.NAME + " available!"));
		...
		{
			event.player.addChatMessage(new TextComponentString(TextFormatting.RED + "An error has occured while checking " + AdvDirector.NAME + " version!"));
			AdvDirector.logger.log(Level.ERROR, "An error has occured while checking " + AdvDirector.NAME + " version!", e);
			return;
		}
	}
