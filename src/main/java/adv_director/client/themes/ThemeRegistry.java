package adv_director.client.themes;
...
import adv_director.api.client.themes.IThemeLoader;
...
public class ThemeRegistry implements IThemeRegistry
{
	...
	private final ITheme fallbackTheme = new ThemeStandard("Standard", new ResourceLocation(AdvDirector.MODID, "textures/gui/editor_gui.png"), new ResourceLocation(AdvDirector.MODID + ":fallback"));
	...
	public void setCurrentTheme(ITheme theme)
	{
		if(theme == null)
		{
			AdvDirector.logger.log(Level.WARN, "Tried to set theme to NULL");
			return;
		}
		...
		if(ConfigHandler.config != null)
		{
			...
			AdvDirector.logger.log(Level.INFO, "Theme set to " + currentTheme.getDisplayName());
		} else
		{
			AdvDirector.logger.log(Level.WARN, "Unable to save theme setting");
		}
	}
                ...
                while (iterator1.hasNext())
                {
                    ...
                    try
                    {
                        ...
                        for(JsonElement je : jAry)
                        {
                        	...
                        	{
                        		AdvDirector.logger.log(Level.WARN, "Invalid theme in " + domain);
                        		continue;
                        	}
                    ...
                    {
                        AdvDirector.logger.log(Level.ERROR, "Error reading bq_themes.json from " + domain, e);
                    }
                }
            } catch (Exception e){}
        }
