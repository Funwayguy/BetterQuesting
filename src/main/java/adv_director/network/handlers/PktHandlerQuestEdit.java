...
public class PktHandlerQuestEdit implements IPacketHandler
{
	...
	public void handleServer(NBTTagCompound data, EntityPlayerMP sender)
	{
		...
		if(!isOP)
		{
			AdvDirector.logger.log(Level.WARN, "Player " + sender.getName() + " (UUID:" + QuestingAPI.getQuestingUUID(sender) + ") tried to edit quest without OP permissions!");
		...
		} else if(action == EnumPacketAction.REMOVE)
		{
			if(quest == null || qID < 0)
			{
				AdvDirector.logger.log(Level.ERROR, sender.getName() + " tried to delete non-existent quest with ID:" + qID);
				return;
			}
			
			AdvDirector.logger.log(Level.INFO, "Player " + sender.getName() + " deleted quest " + quest.getUnlocalisedName());
