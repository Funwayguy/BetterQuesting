...
public class PacketQuesting implements IMessage
{
	...
	public static class HandleServer implements IMessageHandler<PacketQuesting, IMessage>
	{
		...
		public IMessage onMessage(PacketQuesting packet, MessageContext ctx)
		{
			...
			{
				AdvDirector.logger.log(Level.ERROR, "A critical NPE error occured during while handling a BetterQuesting packet server side", new NullPointerException());
				return null;
			}
			...
			{
				AdvDirector.logger.log(Level.WARN, "Recieved a packet server side without an ID");
				return null;
			}
			...
			if(handler == null)
			{
				AdvDirector.logger.log(Level.WARN, "Recieved a packet server side with an invalid ID: " + message.getString("ID"));
				return null;
			} else if(sender != null)
			{
	...
	public static class HandleClient implements IMessageHandler<PacketQuesting, IMessage>
	{
		...
		public IMessage onMessage(PacketQuesting packet, MessageContext ctx)
		{
			if(packet == null || packet.tags == null)
			{
				AdvDirector.logger.log(Level.ERROR, "A critical NPE error occured during while handling a BetterQuesting packet client side", new NullPointerException());
				return null;
			}
