...
import adv_director.api.questing.tasks.IProgression;
...
import adv_director.misc.UserEntry;
...
import adv_director.questing.rewards.RewardStorage;
import adv_director.questing.tasks.TaskStorage;
...
public class QuestInstance implements IQuest
{
	...
	private void readFromJson_Progress(JsonObject json)
	{
		...
		for(JsonElement entry : JsonHelper.GetArray(json, "completed"))
		{
			...
			{
				AdvDirector.logger.log(Level.ERROR, "Unable to load UUID for quest", e);
			}
		}
